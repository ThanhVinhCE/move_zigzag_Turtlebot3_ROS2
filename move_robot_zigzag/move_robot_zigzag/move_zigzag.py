...
class SubscriberClass(Node):
    ...
    def rotate_clockwise(self, desired_angle):
        ...
        self.get_logger().info(f'initial_yaw = {initial_yaw}')
        while self.first_call:
            if initial_yaw <= 0:
                if self.target_angle < -np.pi:
                    self.target_angle1 = np.pi + initial_yaw
                    self.target_angle2 = np.pi - (desired_angle*np.pi/180.0 - self.target_angle1)
                    if abs(self.target_angle1 + np.pi) < 0.005:
                        if not self.publisher.rotating_clockwise_small_angle:
                            self.publisher.rotate_clockwise_small_angle()
                        else:
                            if self.yaw >= 0:
                                self.publisher.rotating_clockwise_small_angle = False
                                self.target_angle2 -= 0.005
        ...
        while True:
            ...
            if initial_yaw <= 0:
                if self.target_angle < -np.pi:
                    if self.rotate_part1_completed:
        ...
        while self.first_call:
            if initial_yaw < 0:
                if self.target_angle > 0:
                    self.target_angle1 = initial_yaw
                    self.target_angle2 = desired_angle*(np.pi)/180.0 + self.target_angle1
                    if abs(self.target_angle1) < 0.005:
                        if not self.publisher.rotating_counter_clockwise_small_angle:
                            self.publisher.rotate_counter_clockwise_small_angle()
                        else:
                            if self.yaw >= 0:
                                self.publisher.rotating_counter_clockwise_small_angle = False
                                self.target_angle2 -= 0.005
                                self.publisher.stop_robot()
                                self.rotate_part1_completed = True
                                self.first_call = False
        ...
        while self.move_zigzag_completed == False:
            ...
            if self.state == 'STOPPED':
                ...
                if self.can_move_straight and \
                    ...
                    not self.pausing_move_straight and not self.start_moving_backward_1s:
                        ...
                        (self.not_rotating_clk_yet or \
                            (self.move_straight_in_a_period_time_finished and self.rotated_clk_first_time)) and \
                                not self.rotated_counter_clk_first_time:
                        ...
                        (self.not_rotating_counter_clk_yet or \
                            (self.move_straight_in_a_period_time_finished and self.rotated_counter_clk_first_time)) and \
                                not self.rotated_clk_first_time:
                ...
                if self.publisher.time_up:
                    ...
                    self.publisher.move_backward_in_a_period_time(3)
...
class PublisherClass(Node):
    ...
    def __init__(self):
        ...
        self.rotating_counter_clockwise_small_angle = False
        self.rotating_clockwise_small_angle = False
        self.time_up = False
        self.linear_velocity = 0.07
        # self.angular_velocity = 1.0
    
    def move_straight_in_a_period_time(self, period_time, can_move_straight):
    ...
    def wait_for_20s_before_move(self):
        self.get_logger().info('In wait_for_20s_before_move function')
        count = 0
        while(count < 20):
            count += 1
            time.sleep(1.0)
    ...
    def move_backward_in_a_period_time(self, time):
        for i in range(time):
            self.move_backward_1s()
    ...
    def rotate_clockwise_small_angle(self):
        ...
        self.rotating_clockwise_small_angle = True
        self.cmd.linear.x = 0.0
    ...
    def rotate_counter_clockwise_small_angle(self):
        ...
        self.rotating_counter_clockwise_small_angle = True
        self.cmd.linear.x = 0.0
        self.cmd.angular.z = 0.01
        self.vel_pub.publish(self.cmd)
